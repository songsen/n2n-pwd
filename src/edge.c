                                 ...
                                 const n2n_sock_t * supernode)
{
    ...
    memcpy( reg.account, eee->account, N2N_ACCOUNT_SIZE );
...
int real_main(int argc, char* argv[])
{
    ...
    while((opt = getopt_long(effectiveargc,
                             effectiveargv,
                             "K:k:a:bc:Eu:g:m:M:s:d:l:L:i:p:fvhrt:RA:", long_options, NULL)) != EOF)
    {
        switch (opt)
        {
        case'K':
       ...
       case 'A':
       {
            ...
            strncpy( eee.account, optarg, N2N_ACCOUNT_SIZE );
            break;
       }
